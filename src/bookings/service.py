...
from bookings.exceptions import InvalidBookingDate, BookingAlreadyBooked, BookingNotFound
from bookings.models import BookingModel
from database import async_session_maker
from rooms.exceptions import RoomNotFound, RoomCannotBeBooked
...
class BookingService(BaseService):
    ...
    @classmethod
    async def check_available_booking(cls,
                                      room_id: int,
                                      date_from: date,
                                      date_to: date,
    ...
    async def update(cls, item_id: int, **data):
        async with async_session_maker() as session:
            ...
            exists_booking = await cls.find_one_or_none(id=item_id,
                                                        user_id=user_id
                                                        )
