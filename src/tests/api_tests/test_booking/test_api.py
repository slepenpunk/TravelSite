...
@pytest.mark.asyncio
async def test_create_booking(room_id,
                              date_from,
                              date_to,
                              status_code,
                              auth_ac: AsyncClient):
    get_me = await auth_ac.get("/users/me")
    user_id = get_me.json()["id"]
    response = await auth_ac.post("/bookings/create", json={
        "user_id": user_id,
        "room_id": room_id,
        "date_from": date_from,
        "date_to": date_to
    ...
    if response.status_code == 200:
        bookings = await auth_ac.get("/bookings")
...
@pytest.mark.parametrize("booking_id,room_id,date_from,date_to,status_code", [
...
@pytest.mark.asyncio
async def test_update_booking(booking_id,
                              room_id,
                              date_from,
                              date_to,
                              status_code,
    ...
    get_me = await auth_ac.get("/users/me")
    user_id = get_me.json()["id"]
    updating_booking = {"booking_id": booking_id,
                        "user_id": user_id,
                        "room_id": room_id,
                        "date_from": date_from,
                        "date_to": date_to}

    response = await auth_ac.patch(f"/bookings/update/{booking_id}", json=updating_booking)
    assert response.status_code == status_code
    if response.status_code == 200:
        get_bookings = await auth_ac.get("/bookings")
